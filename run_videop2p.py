...
class LocalBlend:
    ...
    def __init__(
        self, prompts: List[str], words: List[List[str]], substruct_words=None, start_blend=0.2, th=(.3, .3),
        tokenizer=None,
    ):
...
def make_controller(prompts: List[str], is_replace_controller: bool, cross_replace_steps: Dict[str, float], self_replace_steps: float, blend_words=None, equilizer_params=None, mask_th=(.3,.3), tokenizer=None) -> AttentionControlEdit:
    if blend_words is None:
        lb = None
    else:
        lb = LocalBlend(prompts, blend_words, th=mask_th, tokenizer=tokenizer)
    if is_replace_controller:
        controller = AttentionReplace(prompts, NUM_DDIM_STEPS, cross_replace_steps=cross_replace_steps, self_replace_steps=self_replace_steps, local_blend=lb, tokenizer=tokenizer)
    else:
        controller = AttentionRefine(prompts, NUM_DDIM_STEPS, cross_replace_steps=cross_replace_steps, self_replace_steps=self_replace_steps, local_blend=lb, tokenizer=tokenizer)
...
def load_512_seq(image_path, left=0, right=0, top=0, bottom=0, n_sample_frame=8, sampling_rate=1):
    images = []
...
class NullInversion:
    ...
    def image2latent(self, image):
        with torch.no_grad():
            ...
            if type(image) is torch.Tensor and image.dim() == 4:
                ...
                image = image.permute(2, 0, 1).unsqueeze(0).to(device, dtype=self.weight_dtype)
    ...
    def image2latent_video(self, image):
        with torch.no_grad():
            image = torch.from_numpy(image).float() / 127.5 - 1
            image = image.permute(0, 3, 1, 2).to(device).to(device, dtype=self.weight_dtype)
            latents = self.model.vae.encode(image)['latent_dist'].mean
    ...
    def invert(self, image_path: str, prompt: str, offsets=(0,0,0,0), num_inner_steps=10, early_stop_epsilon=1e-5, verbose=False, n_sample_frame=8):
        self.init_prompt(prompt)
        ptp_utils.register_attention_control(self.model, None)
        image_gt = load_512_seq(image_path, *offsets, n_sample_frame=n_sample_frame)
        if verbose:
            print("DDIM inversion...")
        image_rec, ddim_latents = self.ddim_inversion(image_gt)
        ddim_latents = [l.to(dtype=torch.float32) for l in ddim_latents]
        if verbose:
            print("Null-text optimization...")
        uncond_embeddings = self.null_optimization(ddim_latents, num_inner_steps, early_stop_epsilon)
        uncond_embeddings = [l.to(dtype=self.weight_dtype) for l in uncond_embeddings]
        return (image_gt, image_rec), ddim_latents[-1], uncond_embeddings

    def invert_(self, image_path: str, prompt: str, offsets=(0,0,0,0), num_inner_steps=10, early_stop_epsilon=1e-5, verbose=False, n_sample_frame=8):
        self.init_prompt(prompt)
        ptp_utils.register_attention_control(self.model, None)
        image_gt = load_512_seq(image_path, *offsets, n_sample_frame=n_sample_frame)
        if verbose:
    ...
    def __init__(self, model, weight_dtype):
...
def main(
    ...
    null_inversion = NullInversion(ldm_stable, weight_dtype)
    ...
    controller = make_controller(prompts, is_word_swap, cross_replace_steps, self_replace_steps, blend_word, eq_params, mask_th=mask_th, tokenizer=tokenizer)
